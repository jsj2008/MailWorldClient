...
#define BOTTOM_MARGIN_DEFAULT 8.0f
#define FONT_SIZE_DEFAULT 13.0f
#define HORIZONTAL_PADDING_DEFAULT 8.0f
#define VERTICAL_PADDING_DEFAULT 8.0f
#define BACKGROUND_COLOR [UIColor colorWithRed:0.976471 green:0.976471 blue:0.976471 alpha:1]
#define TEXT_COLOR [UIColor blackColor]
#define HIGHLIGHTED_TEXT_COLOR [UIColor whiteColor]
...
- (void)touchedTag:(id)sender
{
    ...
    if(tagView && self.tagDelegate && [self.tagDelegate respondsToSelector:@selector(selectedTag:tag:)])
        [self.tagDelegate selectedTag:tagView.label.text tag:self.tag];
}
...
- (void)display
{
    ...
    for (NSString *text in textArray) {
        ...
        if (tagViews.count > 0) {
            ...
            tagView = [[DWTagView alloc] initWithBorderColor:self.borderColor];
        }
...
- (void)touchUpInside:(id)sender
{
    ...
    if(button && self.tagDelegate && [self.tagDelegate respondsToSelector:@selector(selectedTag:tag:)])
        [self.tagDelegate selectedTag:button.accessibilityLabel tag:self.tag];
}
...
- (id)initWithBorderColor:(UIColor *)color {
    self = [super init];
    if (self) {
        ...
        _button.titleLabel.highlightedTextColor = HIGHLIGHTED_TEXT_COLOR;
