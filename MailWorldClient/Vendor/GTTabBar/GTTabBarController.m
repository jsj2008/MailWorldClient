...
- (id)initWithViewControllers:(NSArray *)vcs imageArray:(NSArray *)arr
{
	...
	if (self != nil)
	{
        ...
        NSLog(@"%f;%f;%f;%f", [[UIScreen mainScreen] applicationFrame].origin.x, [[UIScreen mainScreen] applicationFrame].origin.y, [[UIScreen mainScreen] applicationFrame].size.width, [[UIScreen mainScreen] applicationFrame].size.height);
...
- (void)displayViewAtIndex:(NSUInteger)index
{
    ...
    {
        if (![self.delegate tabBarController:self shouldSelectViewController:[self.viewControllers objectAtIndex:index] index:index])
        {
            return;
        }
    }
...
//        selectedVC.view.frame = transitionView.frame;
        selectedVC.view.frame = CGRectMake(0, 0, 320, transitionView.frame.size.height);
        ...
        {
            [self.delegate tabBarController:self didSelectViewController:selectedVC index:index];
        }
    }
	else {
