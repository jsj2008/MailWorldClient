...
//        CGFloat cropHeight = theFrame.size.height/2-captureVideoHeight/2-44;
        
        if (!oneDMode) {
            cropRect = CGRectMake(kPadding, (self.frame.size.height - rectSize) / 2-44, rectSize, rectSize);
        } else {
...
- (void)drawRect:(CGRect)rect {
    ...
    if (displayedMessage == nil) {
        self.displayedMessage = NSLocalizedStringWithDefaultValue(@"OverlayView displayed message", nil, [NSBundle mainBundle], @"将二维码放入框内，即可自动扫描", @"将二维码放入框内，即可自动扫描");
    }
	CGContextRef c = UIGraphicsGetCurrentContext();
//  [UIColor colorWithRed:0.094118 green:0.094118 blue:0.094118 alpha:1]
...
//        CGRect displayRect = CGRectMake((rect.size.width - displaySize.width) / 2 , cropRect.origin.y - displaySize.height, displaySize.width, displaySize.height);
        CGRect displayRect = CGRectMake((rect.size.width - displaySize.width) / 2 , cropRect.origin.y + cropRect.size.height + displaySize.height + 8, displaySize.width, displaySize.height);
