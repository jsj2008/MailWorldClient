...
#define kDShopImageWidth 72.0
#define kDShopImageHeight 72.0

@implementation DiscountsShopCell
...
- (void)setShop:(DiscountsShopEntity *)shop_ {
    shop = shop_;
...
+ (UIFont *)newPriceFont {
    return [UIFont systemFontOfSize:32.0f];
}
...
+ (CGFloat)heightForEntry:(DiscountsShopEntity *)entity withWidth:(CGFloat)width {
    ...
    return kDShopImageHeight + 24.0f;
}
...
- (void)layoutSubviews {
    ...
    self.imageView.frame = CGRectMake(offsets.width, offsets.height, kDShopImageWidth, kDShopImageHeight);
...
#pragma mark - 画门店图片
    ...
    shopImageView.imageUrl = [shop hdimg];
    CGRect r = CGRectMake(offsets.width, offsets.height, kDShopImageWidth, kDShopImageHeight);
    [shopImageView.image drawInRect:r];
    
    NSString *hdname = [shop hdname];
    NSString *hdcontent = [shop hdcontent];
//    NSString *begantime = [shop begantime];
...
//    [shopname drawInRect:CGRectMake(offsets.width*2+kDShopImageWidth, offsets.height, bounds.width - (2 * offsets.width), 21) withFont:[[self class] shopNameFont] lineBreakMode:NSLineBreakByWordWrapping];
//#pragma mark - 画距离
...
//        o2orect = CGRectMake(offsets.width*3+kDShopImageWidth+zitirect.size.width, offsets.height*2+[o2o sizeWithFont:[[self class] serverFont]].height, o2owidth, 21);
//        o2obgrect = CGRectMake(offsets.width*3+kDShopImageWidth+zitirect.size.width, offsets.height*2+[o2o sizeWithFont:[[self class] serverFont]].height-2, o2owidth, 21);
//    }
//    else {
...
//        o2obgrect = CGRectMake(offsets.width*3+kDShopImageWidth+zitirect.size.width, offsets.height*2+[o2o sizeWithFont:[[self class] serverFont]].height-2, -offsets.width, 0);
//    }
//    
//    //选择或高亮时对应的颜色
...
//#pragma mark - 画星级
//    if (!([self isHighlighted] || [self isSelected])) [[UIColor lightGrayColor] set];
//    CGRect starrect = CGRectMake(offsets.width*4+kDShopImageWidth+o2orect.size.width+zitirect.size.width, offsets.height*2+[shopname sizeWithFont:[[self class] serverFont]].height, 16*5+2*4, 16);
//    
